...
"""
To run: python3 eval_scrubber_ucsf.py -p ./scrubber_out_ucsf_2k -a ./ucsf_anno -o ./ucsf_results
...
def comparison(filename, file1path, file2path, allpositive_dict):
    ...
    ucsf_include_tags = ['Date','Provider_Name','Phone_Fax','Patient_Name_or_Family_Member_Name','Patient_Address','Provider_Address_or_Location','Provider_Certificate_or_License','Patient_Medical_Record_Id','Patient_Account_Number','Patient_Social_Security_Number','Patient_Vehicle_or_Device_Id','Patient_Unique_Id','Email','URL_IP','Patient_Biometric_Id_or_Face_Photo','Patient_Certificate_or_License','Age','Patient_Initials','Provider_Initials']
    ...
    for tag in fn_dict:
        # First deal with ages
        if tag == 'Age':
            for fn in fn_dict_copy[tag]:
                if fn.isdigit():
                    # If this age < 90, remove it from teh list of fns
                    if int(fn) < 90:
    ...
    all_true_ucsf_fns = sum([len(fn_dict_copy[key]) for key in fn_dict_copy])
        ...
        for tag in fn_dict_copy:
            if fn in fn_dict_copy[tag]:
                ...
                if tag in ucsf_include_tags:
                    # If age, make sure the age is >90
                    if tag == 'Age':
                        if fn.isdigit():
                            # If this age >= 90, add it to the list of fns
                            if int(fn) >= 90:
                                # Then add this word to the fn dict
                                ucsf_category_fn_dict[tag].append(fn)
                                all_ucsf_fns.append(fn)
    ...
    # print(file_context_dict)
    # Calculate true positives
    true_positive = all_true_ucsf_fns - len(all_ucsf_fns)
    summary_dict['true_positive'] = true_positive
    summary_dict['false_negative'] = all_ucsf_fns
    ...
    output += '\n'
    #print(summary_dict)
    return summary_dict, output, ucsf_category_fn_dict, file_context_dict
...
def main():
    ...
    allpositive_dict = json.loads(open('/data/muenzenk/nlm_scrubber/ucsf_detailed_anno_dict.json').read())


    all_ucsf_fn_dict = {'Date':0,
    'Provider_Name':0,
    'Phone_Fax':0,
    'Age':0,
    'Patient_Name_or_Family_Member_Name':0,
    'Patient_Address':0,
    'Patient_Initials':0,
    'Provider_Address_or_Location':0,
    'Provider_Initials':0,
    ...
    'Email':0,
    'URL_IP':0,
    'Patient_Certificate_or_License':0
    }
    ...
    if os.path.isfile(file1path) != os.path.isfile(file2path):
        ...
        if os.path.isfile(file1path):
            ...
            if file1name != file2name:
                ...
                summary_text += output
                if_update = True
                for tag in all_ucsf_fn_dict:
                    all_ucsf_fn_dict[tag] += len(file_fn_dict[tag])
        else:
        ...
        print('\n')
        for tag in all_ucsf_fn_dict:
            print(tag + ': ' + str(all_ucsf_fn_dict[tag]))
        if if_update:
